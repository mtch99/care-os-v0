...
describe('initializeChartNote', () => {
  ...
  describe('Given concurrent double-tap causes unique violation, when initializing', () => {
    it('then re-SELECTs and returns existing with created: false', async () => {
      // Simulate: first request inserts successfully, second hits unique violation
      const existingRow: ChartNoteRow = {
        id: 'race-winner-id',
        sessionId: SESSION_ID,
        templateVersionId: TEMPLATE_ID,
      ...
      // The repo will throw unique violation on insert, but the re-SELECT finds the row
      chartNoteRepo.simulateUniqueViolationOnNextInsert()
      chartNoteRepo.seed(existingRow)

      const result = await initializeChartNote(makeInput(), makePorts())

      expect(result.created).toBe(false)
      expect(result.chartNote.id).toBe('race-winner-id')
    })

    it('then does not emit events on race-condition fallback', async () => {
      const existingRow: ChartNoteRow = {
        id: 'race-winner-id',
        sessionId: SESSION_ID,
        templateVersionId: TEMPLATE_ID,
