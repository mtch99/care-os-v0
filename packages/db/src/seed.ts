...
async function seed() {
  ...
  const today10 = new Date(today); today10.setHours(10);
  const today11 = new Date(today); today11.setHours(11);

  await db.insert(appointments).values([
    { id: APPT_1_ID, clinicId: CLINIC_ID, patientId: ALICE_ID, practitionerId: PHYSIO_ID, appointmentType: 'initial',   status: 'scheduled', scheduledAt: today },
    { id: APPT_2_ID, clinicId: CLINIC_ID, patientId: BOB_ID,   practitionerId: ERGO_ID,   appointmentType: 'follow_up', status: 'scheduled', scheduledAt: today10 },
    { id: APPT_3_ID, clinicId: CLINIC_ID, patientId: ALICE_ID, practitionerId: PHYSIO_ID, appointmentType: 'follow_up', status: 'canceled',  scheduledAt: today11 },
  ]).onConflictDoNothing();

  // Chart note templates
  await db.insert(chartNoteTemplates).values([
    { id: TEMPLATE_1_ID, name: 'IAF — Physiotherapy',        discipline: 'physiotherapy', appointmentType: 'initial'   },
    { id: TEMPLATE_2_ID, name: 'SOAP Note — Physiotherapy',  discipline: 'physiotherapy', appointmentType: 'follow_up' },
    { id: TEMPLATE_3_ID, name: 'IAF — Ergotherapy',          discipline: 'ergotherapy',   appointmentType: 'initial'   },
    { id: TEMPLATE_4_ID, name: 'SOAP Note — Ergotherapy',    discipline: 'ergotherapy',   appointmentType: 'follow_up' },
  ]).onConflictDoNothing();
