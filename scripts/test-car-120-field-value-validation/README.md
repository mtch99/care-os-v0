...
| 01 | `01-happy-path.sh <id> <v>` | **200** | Valid payload across `text`, `scale`, `select`, `date`, `narrative` → 200, version bumps. The baseline sanity check |
...
| 05 | `05-select-not-in-options.sh <id> <v>` | **422** `NOT_IN_OPTIONS` | `mechanism_of_injury: "Unicorn"` doesn't match any declared option |
| 06 | `06-select-accepts-fr-locale.sh <id> <v>` | **200** | `mechanism_of_injury: "Traumatique"` — documents locale-pinned matching. Persisted value IS the FR string; downstream consumers must handle both locales until [CAR-122](https://linear.app/careos/issue/CAR-122) lands. Version bumps |
...
The three that bump (01, 06, 12) should be run sequentially with the new version from the previous step. Each script echoes `Next VERSION=…` on success.
...
./06-select-accepts-fr-locale.sh $CN $V && V=3
...
**No garbage persisted** — every saved scale is within `[0, 10]`, every select is one of the declared option strings (fr or en):
...
**Event audit** — `chartNote.saved` events fired only for the three successful saves (01, 06, 12). If you have an event log table, query it; if events are Inngest-only, check the Inngest dev server UI at `http://localhost:9376`.
