...
- **Idempotent inserts (insert-or-return-existing).** For commands like `initialize` or `create`, the insert port returns `{ row, created: boolean }` — not exceptions. The domain pattern-matches on `created`, never inspects raw DB errors. Events fire only when `created === true`. Keep a pre-insert lookup (e.g. `findBySessionId`) for efficiency — it avoids aggregate construction on repeat calls. The port's `{ created: false }` handles the race; the pre-check handles the common non-concurrent case. See `docs/solutions/integration-issues/drizzle-error-wrapping-domain-isolation.md` for the full pattern.
